...
export default function App() {
  ...
  useEffect(() => {
    ...
    const runCheck = () => {
      ...
      const enabled = localStorage.getItem(`sgr_notify_enabled_${currentUser.email}`) === 'true';
      if (!enabled) return;

      const timing = localStorage.getItem(`sgr_notify_timing_${currentUser.email}`) || 'vespera';
      const timeStr = localStorage.getItem(`sgr_notify_time_${currentUser.email}`) || '19:00';
