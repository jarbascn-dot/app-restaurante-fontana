...
export default function App() {
  ...
  useEffect(() => {
    ...
    if ('serviceWorker' in navigator) {
      navigator.serviceWorker.register('/sw.js')
        ...
        .then(reg => { if (reg.active) reg.active.postMessage({ type: 'FIREBASE_CONFIG', config: window.__FIREBASE_CONFIG__ }); })
  ...
  useEffect(() => {
    if (isLogged && currentUser && currentUser.email) {
      // PRIMARY: Register FCM token for server-side push (Android background notifications)
      const userEmail = currentUser.email || currentUser.cpf || '';
      if (userEmail) registerFCMToken(userEmail).catch(() => {});
  ...
  const handleClearAllReservas = async (mode: 'all' | 'future') => {
    ...
    const confirm = window.confirm(`ATENÇÃO CORP: Você selecionou apagar permanentemente ${toDelete.length} ${modeText}.\nDeseja prosseguir com essa exclusão definitiva no banco de dados Firestore?`);
    if (!confirm) return;
  ...
  const handleReset = () => {
    const confirm = window.confirm('Deseja realmente limpar todas as alterações e recarregar os dados padrões para demonstração?');
    if (!confirm) return;
