...
export default function App() {
  ...
  const handleUpdatePassword = (newSenha: string) => {
    const hashed = hashPassword(newSenha);
    const updatedUser = { ...currentUser, senha: hashed };
      ...
      if (u.id === id) {
        if (extraData && status === UserStatus.Aprovado) {
          return { 
            ...
            senha: hashPassword(extraData.senha || u.senha || '1234@'),
  ...
  const handleToggleUserActive = async (id: string) => {
    ...
    const nextStatus = wasDeactivated ? UserStatus.Desativado : UserStatus.Aprovado;
    const todayDate = getTodayDate();

    const updatedUser = { ...userObj, status: nextStatus };

    setUsuarios(prev => prev.map(u => u.id === id ? updatedUser : u));
    await saveToFirestore('usuarios', updatedUser);

    // Se o colaborador foi desativado, remove/cancela todas as reservas futuras para evitar cobranças futuras
    if (wasDeactivated) {
    ...
    const opMessage = wasDeactivated 
      ? `Usuário DESATIVADO: ${userObj.nome}. Reservas futuras canceladas e histórico mantido para faturamento.`
      : `Usuário REATIVADO: ${userObj.nome}`;

    appendAuditLog(opMessage);
    triggerFlashNotification(wasDeactivated ? `Colaborador desativado. Reservas futuras removidas e histórico mantido para o faturamento.` : `Colaborador reativado!`);
  };
  ...
  const handleSaveUser = async (updatedUser: Usuario) => {
    const originalUser = usuarios.find(u => u.id === updatedUser.id);
    let finalPassword = updatedUser.senha;

    if (originalUser) {
      if (!finalPassword) {
        // If password is blank/empty, retain original
        finalPassword = originalUser.senha || '1234@';
      } else {
      ...
      // New direct addition
      finalPassword = hashPassword(finalPassword || '1234@');
    }

    const finalUserObj = { ...updatedUser, senha: finalPassword };

    setUsuarios(prev => {
      const exists = prev.some(u => u.id === finalUserObj.id);
      if (exists) {
        return prev.map(u => u.id === finalUserObj.id ? finalUserObj : u);
      } else {
        return [...prev, finalUserObj];
      }
    });
    saveToFirestore('usuarios', finalUserObj);
    
    if (currentUser?.id === finalUserObj.id) {
      setCurrentUser(finalUserObj);
    }
    
    appendAuditLog(`Usuário salvo pelo RH: ${finalUserObj.nome} (${finalUserObj.email}) - Perfil: ${finalUserObj.perfil}`);
    triggerFlashNotification(`Colaborador ${finalUserObj.nome} salvo com sucesso!`);
  };
  ...
  const handleResetUserPassword = (userId: string, tempPass: string) => {
    ...
    const hashedTemp = hashPassword(tempPass);
    setUsuarios(prev => prev.map(u => {
      if (u.id === userId) {
        targetUser = { ...u, senha: hashedTemp };
        return targetUser;
      }
      return u;
    }));
  ...
  const handleRegisterUser = (newUser: Usuario, finishMessage: string) => {
    const hashedUser = { ...newUser, senha: hashPassword(newUser.senha) };
    setUsuarios(prev => [...prev, hashedUser]);
    saveToFirestore('usuarios', hashedUser);
