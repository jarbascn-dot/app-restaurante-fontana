...
// CORREÇÃO: Isola o saveTokenToFirestore em try/catch próprio.
// Se as regras de segurança do Firestore bloquearem a escrita direta
// em usuarios/{email} (ID do doc ≠ UID do Firebase Auth), o erro NÃO
// deve impedir o retorno do token. Os chamadores (registerFCMToken,
// scheduleNotification, autoRecoverFCMToken) salvam o token via
// /api/update-fcm-token (Admin SDK) que bypassa as regras de segurança.
if (userEmail) {
try {
await saveTokenToFirestore(token, userEmail);
} catch (saveErr) {
console.warn('[FCM] Aviso: falha ao salvar token diretamente no Firestore (token retornado para salvar via /api/update-fcm-token):', saveErr);
}
}
return token;
} catch (err: any) {
...
console.warn('[FCM] Aviso (retry): falha ao salvar token diretamente no Firestore:', saveErr);
}
}
return token;
} catch (retryErr) {
