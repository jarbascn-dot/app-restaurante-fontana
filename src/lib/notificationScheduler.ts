...
export async function scheduleNotification(
  ...
  try {
      ...
      // Salvar no Firestore via API do servidor para o cron funcionar com app fechado
      if (email) {
              // Get FCM token to pass along (optional - improves reliability)
...
export async function registerFCMToken(userId: string): Promise<void> {
    try {
          ...
          // Register via API endpoint (server-side, bypasses Firestore rules)
      const response = await fetch('/api/register-notification', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ userId, time: '00:00', title: '', body: '', token }),
      });
          if (response.ok) {
                  console.log('[FCM] Token registered successfully for user:', userId);
          } else {
                  console.warn('[FCM] Failed to register token via API:', response.status);
          }
    } catch (err) {
