...
/**
 * Persist the notification schedule to Firestore (notificationQueue).
 * This ensures the scheduledTime is always up-to-date when the user changes the time.
 */
export async function syncNotificationQueueToFirestore(
    email: string,
    ...
    try {
          const docId = `daily_${email.replace(/[^a-zA-Z0-9]/g, '_')}`;
          const docRef = doc(db, 'notificationQueue', docId);
          await setDoc(docRef, {
                  userId: email,
                  scheduledTime,
                  title,
                  ...
                  daily,
                  link: '/',
                  sent: false,        // Reset sent so it will be processed at the new time
                  updatedAt: serverTimestamp(),
                  // Clear any previous skip info
                  skipReason: null,
                  skippedAt: null,
          }, { merge: true });
          console.log(`[Scheduler] notificationQueue updated: ${docId} -> scheduledTime: ${scheduledTime}`);
    } catch (err) {
          console.error('[Scheduler] Failed to sync notificationQueue to Firestore:', err);
    }
}
...
export async function scheduleNotification(
  ...
  try {
      ...
      // Sync to Firestore so server-side cron can send push when app is closed
      if (email) {
              await syncNotificationQueueToFirestore(email, time, title, body);
      }
