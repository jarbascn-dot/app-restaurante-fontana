...
export default function AccountSettingsModal({
  ...
  // Sync state whenever currentUser notification fields are updated
  useEffect(() => {
    if (currentUser) {
      if (currentUser.alertaEnabled !== undefined) setNotifyEnabled(currentUser.alertaEnabled);
      if (currentUser.alertaTiming) setNotifyTiming(currentUser.alertaTiming);
      if (currentUser.alertaTime) setNotifyTime(currentUser.alertaTime);
      if (currentUser.alertaTipo) setNotifyTipo(currentUser.alertaTipo);
      if (currentUser.alertaChannel) setNotifyChannel(currentUser.alertaChannel);
    }
  }, [currentUser.alertaEnabled, currentUser.alertaTiming, currentUser.alertaTime, currentUser.alertaTipo, currentUser.alertaChannel]);
