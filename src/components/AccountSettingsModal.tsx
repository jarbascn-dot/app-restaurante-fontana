...
interface AccountSettingsModalProps {
  ...
  onUpdateNotifications?: (enabled: boolean, timing: 'vespera' | 'mesmo_dia', time: string) => void;
  ...
  const handleSaveNotifySettings = () => {
    ...
    if (notifyEnabled) {
      ...
      scheduleNotification(
        notifyTime,
        'SGR Fontana',
        `Lembrete: consulte e agende sua refeição antes das ${settings.horarioLimite}!`,
        currentUser.email
      );
    }

    if (onUpdateNotifications) {
      onUpdateNotifications(notifyEnabled, notifyTiming, notifyTime);
    } else {
      appendAuditLog(
            ...
            <div className="space-y-4 pt-1">
              ...
              {notifyEnabled && (
                <div className="space-y-4 border-l-2 border-emerald-500 pl-4 py-1 animate-[fadeIn_0.25s_ease] space-y-3">
                  ...
                  <div className="bg-neutral-950 text-white p-4 rounded-xl border border-neutral-800 space-y-3 shadow-inner">
                    <div className="text-left">
                      <span className="font-bold text-neutral-100 text-xs flex items-center gap-1">
                        <Sparkles className="h-4 w-4 text-emerald-400" /> Laboratório de Testes de Alerta
                      </span>
                      <p className="text-[10px] text-neutral-400 mt-0.5">
                        Verifique o comportamento e os novos ícones estilo WhatsApp das notificações neste aparelho.
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                      <button
                        ...
                        className="px-3 py-2 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-bold rounded-lg border border-neutral-700 transition flex items-center justify-center gap-1.5 text-[10px]"
                      >
                        <Volume2 className="h-4 w-4 text-sky-400" /> Simular na Tela (Rápido)
                      </button>

                      <button
                        type="button"
                        disabled={pushStatus !== 'active' || sendingPush}
                        onClick={handleTriggerRealPush}
                        className={`px-3 py-2 text-white font-bold rounded-lg transition flex items-center justify-center gap-1.5 text-[10px] ${
                          pushStatus === 'active' 
                            ? 'bg-emerald-600 hover:bg-emerald-500 shadow-md active:scale-[0.98]' 
                      ...
                      >
                        {sendingPush ? (
                          ...
                          <Smartphone className={`h-4 w-4 ${pushStatus === 'active' ? 'text-emerald-300 animate-pulse' : 'text-neutral-600'}`} />
                        )}
                        Testar Push Real (2º Plano)
                      </button>
                    </div>

                    {pushStatus === 'active' && (
                      <p className="text-[9px] text-emerald-400/90 leading-tight bg-emerald-950/40 p-2 rounded border border-emerald-900/40 text-center font-medium">
                        💡 <strong>Para testar plano de fundo:</strong> Clique no botão verde de Push Real, mude de app ou bloqueie seu celular imediatamente. O servidor disparará o alerta estilo WhatsApp!
                      </p>
                    )}
                  </div>
