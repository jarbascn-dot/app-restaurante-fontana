...
import { LogIn, KeyRound, User, AlertCircle, UserPlus, HelpCircle, Fingerprint, Smartphone, UtensilsCrossed } from 'lucide-react';
...
export default function LoginScreen({ usuarios, settings, onLoginSuccess, onOpenRegister, onResetPassword }: LoginScreenProps) {
  ...
  const handleForgotPasswordClick = () => {
    ...
    if (!cleanId) {
      setError("Por favor, preencha o campo 'CPF ou E-mail' acima para solicitar uma recuperação de senha.");
      return;
    }

    const cleanRaw = cleanId.toLowerCase();
    const cleanNum = cleanRaw.replace(/\D/g, '');
    ...
    if (!foundUser) {
      setError("Não localizamos nenhum cadastro com as informações fornecidas.");
      return;
    }

    if (foundUser.email && foundUser.email.includes('@')) {
      const emailVal = foundUser.email.trim();
      const parts = emailVal.split('@');
      const hiddenEmail = parts[0].slice(0, 3) + "***@" + parts[1];
      
      const firstPartNum = Math.floor(1000 + Math.random() * 9000).toString();
      const tempPass = `FTN${firstPartNum}@`;
      
      onResetPassword(foundUser.id, tempPass);
      
      setSuccessInfo(`Uma senha provisória foi gerada com sucesso e enviada para o seu e-mail cadastrado (${hiddenEmail}). Por favor, verifique sua caixa de entrada ou spam e utilize-a (${tempPass}) para fazer o seu login!`);
    } else {
      setError("Atenção: Não encontramos nenhum e-mail associado a este CPF em nossos cadastros. Por favor, procure pelo setor de Recursos Humanos (RH) da Construtora Fontana.");
    }
  };
