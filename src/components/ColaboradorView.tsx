...
import { Calendar as CalendarIcon, Check, X, ShieldAlert, Clock, RefreshCw, FileText, Download, ExternalLink, AlertTriangle, MousePointerClick, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function ColaboradorView({
  ...
  // Active calendar view month and year states
  ...
  // States for Batch/Period Booking
  const [batchStart, setBatchStart] = useState('2026-06-15'); // Monday next week
  const [batchEnd, setBatchEnd] = useState('2026-06-19'); // Friday next week
  ...
  return (
    <div className="space-y-6" id="colaborador-view-panel">
      ...
      <div className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
        ...
        <div className="bg-neutral-50 px-4 py-3 rounded-lg border border-neutral-200 flex flex-col justify-center">
          ...
          <div className="grid grid-cols-3 gap-2 mt-2 text-center">
            ...
            <div className="p-1.5 bg-neutral-100 text-neutral-800 rounded">
              <span className="block text-sm font-extrabold">{totalCancelados}</span>
              <span className="text-[9px] font-medium text-neutral-500">Cancelas</span>
            </div>
            <div className="p-1.5 bg-blue-50 text-blue-800 rounded">
              <span className="block text-sm font-extrabold">{totalConsumidos}</span>
              <span className="text-[9px] font-medium text-neutral-500">Refeições</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-neutral-200 shadow-sm lg:col-span-2 space-y-4" id="calendar-block">
          <div className="flex justify-between items-center pb-2 border-b border-neutral-100 flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-bold text-neutral-800 text-sm uppercase font-mono tracking-wide">
      ...
      {viewingPdf && (
        <div className="fixed inset-0 bg-neutral-900/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="pdf-viewer-modal">
          <div className="bg-white rounded-xl shadow-2xl border border-neutral-200 w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden animate-scale-up">
            {/* Modal Header */}
            <div className="bg-neutral-950 text-white px-5 py-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                <FileText className="h-5 w-5 text-emerald-400" />
                <div>
                  <h3 className="text-xs sm:text-sm font-black uppercase tracking-wider">{pdfTitle}</h3>
                  <p className="text-[10px] text-neutral-400 leading-none mt-0.5">Visualizador de Documentos Fontana</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <a
                  href={viewingPdf}
                  download={pdfTitle}
                  className="p-1.5 hover:bg-neutral-800 rounded text-neutral-400 hover:text-white transition cursor-pointer"
                  title="Baixar arquivo original"
                >
            ...
            <div className="bg-amber-50 border-b border-amber-200 px-5 py-2.5 text-amber-900 text-[11px] flex flex-wrap items-center justify-between gap-3 font-medium leading-normal">
              <span className="flex items-center gap-1.5">
                <AlertTriangle className="h-3.5 w-3.5 text-amber-600 shrink-0" />
                <span>Navegadores modernos podem bloquear links externos em blocos incorporados. Se a visualização estiver em branco:</span>
              </span>
              <a
                href={viewingPdf}
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-[10px] rounded transition flex items-center gap-1"
              >
                <ExternalLink className="h-3 w-3" /> Abrir em Nova Aba / Baixar PDF
              </a>
            </div>
