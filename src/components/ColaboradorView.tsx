...
export default function ColaboradorView({
  ...
  // Active calendar view month and year states initialized dynamically from todayDate
  ...
  const [currentMonth, setCurrentMonth] = useState<number>(() => {
    if (todayDate) {
      const parts = todayDate.split('-');
      if (parts.length === 3) {
        return parseInt(parts[1], 10) - 1; // 0-indexed: e.g. July (07) is index 6
      }
    }
    return 5; // 0-indexed: 5 is June (fallback)
  });
