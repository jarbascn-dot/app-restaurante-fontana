/**
 * @license
 * SPDX-License-Identifier: Apache-2.0
 */

import React, { useState } from 'react';
import { Usuario, Perfil, UserStatus, Obra, Empresa } from '../types';
import { UserPlus, CheckCircle, AlertTriangle, X, Camera } from 'lucide-react';
import CameraCapture from './CameraCapture';

interface RegisterModalProps {
  isOpen: boolean;
  onClose: () => void;
  obras: Obra[];
  empresas: Empresa[];
  usuarios: Usuario[];
  onRegister: (newUser: Usuario, message: string) => void;
}

export default function RegisterModal({ isOpen, onClose, obras, empresas, usuarios, onRegister }: RegisterModalProps) {
  const [nome, setNome] = useState('');
  const [cpf, setCpf] = useState('');
  const [email, setEmail] = useState('');
  const [senha, setSenha] = useState('');
  const [fotoBiometria, setFotoBiometria] = useState('');
  const [isCapturing, setIsCapturing] = useState(false);
  
  const [successMsg, setSuccessMsg] = useState<{ text: string; autoApproved: boolean } | null>(null);

  if (!isOpen) return null;

  const formatCpf = (value: string) => {
    const digits = value.replace(/\D/g, '').slice(0, 11);
    if (digits.length <= 3) return digits;
    if (digits.length <= 6) return `${digits.slice(0, 3)}.${digits.slice(3)}`;
    if (digits.length <= 9) return `${digits.slice(0, 3)}.${digits.slice(3, 6)}.${digits.slice(6)}`;
    return `${digits.slice(0, 3)}.${digits.slice(3, 6)}.${digits.slice(6, 9)}-${digits.slice(9, 11)}`;
  };

  const handleCpfChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setCpf(formatCpf(e.target.value));
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nome || !cpf || !senha) {
      alert('Por favor, preencha todos os campos obrigatórios (*).');
      return;
    }

    const cleanCpfVal = cpf.replace(/\D/g, '');
    if (cleanCpfVal.length !== 11) {
      alert('Por favor, informe um CPF válido com 11 dígitos.');
      return;
    }

    if (!fotoBiometria) {
      alert('A foto de biometria facial é obrigatória para o cadastro. Por favor, utilize o botão abaixo para abrir a câmera e registrar sua face.');
      return;
    }

    const alreadyExists = usuarios.some(u => u.cpf && u.cpf.replace(/\D/g, '') === cleanCpfVal);
    if (alreadyExists) {
      alert('Este CPF já está cadastrado no sistema. Não é possível efetuar mais de um cadastro.');
      return;
    }

    // All registrations now default to Pending so HR can register their company, matricula, sector, and profile.
    const status = UserStatus.Pendente;

    const newUser: Usuario = {
      id: 'u-' + Math.random().toString(36).substr(2, 9),
      nome,
      cpf: cleanCpfVal,
      email: email.trim() || undefined,
      senha,
      matricula: '', // to be filled by HR on approval
      perfil: Perfil.Colaborador, // to be edited/assigned by HR on approval
      status,
      idEmpresa: '', // to be filled by HR on approval
      idObraPadrao: '', // to be filled by HR on approval
      fotoBiometria,
      criadoEm: new Date().toISOString()
    };

    const msg = `Cadastro solicitado com sucesso! Seu CPF foi registrado como login. Aguarde a aprovação do RH para preenchimento de sua matrícula, empresa, obra e acesso.`;
    setSuccessMsg({ text: msg, autoApproved: false });

    onRegister(newUser, msg);
  };

  const handleFinish = () => {
    setSuccessMsg(null);
    setNome('');
    setCpf('');
    setEmail('');
    setSenha('');
    setFotoBiometria('');
    onClose();
  };

  return (
    <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" id="register-modal-overlay">
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full overflow-hidden border border-neutral-100 flex flex-col" id="register-modal-container">
        
        {/* Header */}
        <div className="bg-neutral-900 text-white p-5 flex justify-between items-center">
          <div className="flex items-center gap-2">
            <UserPlus className="h-5 w-5 text-emerald-400" />
            <h3 className="font-semibold text-lg tracking-tight">Solicitar Cadastro no SGR</h3>
          </div>
          <button 
            onClick={onClose} 
            className="text-neutral-400 hover:text-white transition-colors"
            id="modal-close-btn"
          >
            <X className="h-5 w-5" />
          </button>
        </div>

        {successMsg ? (
          /* Success screen */
          <div className="p-8 text-center flex flex-col items-center gap-4" id="register-success-view">
            {successMsg.autoApproved ? (
              <div className="bg-emerald-100 text-emerald-700 p-4 rounded-full">
                <CheckCircle className="h-12 w-12" />
              </div>
            ) : (
              <div className="bg-amber-100 text-amber-700 p-4 rounded-full">
                <AlertTriangle className="h-12 w-12" />
              </div>
            )}
            
            <h4 className="text-xl font-bold text-neutral-800">
              {successMsg.autoApproved ? 'Acesso Liberado!' : 'Solicitação Recebida'}
            </h4>
            <p className="text-sm text-neutral-600 leading-relaxed max-w-sm">
              {successMsg.text}
            </p>

            <button
              onClick={handleFinish}
              className="mt-4 px-6 py-2.5 bg-neutral-900 text-white rounded-lg hover:bg-neutral-800 transition-colors font-medium text-sm"
              id="success-finish-btn"
            >
              Concluir e Fechar
            </button>
          </div>
        ) : (
          /* Form screen */
          <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto" id="register-form">
            <div className="bg-neutral-50 border-l-4 border-emerald-500 p-3.5 rounded">
              <p className="text-xs text-neutral-600 font-medium">
                💡 **Segurança e Triagem Ativa:** Conforme as diretrizes de compliance, seus dados de **Matrícula**, **Empresa Vinculada**, **Setor/Área (Obra)** e **Perfil de Acesso** serão obrigatoriamente preenchidos e validados pelo RH no momento da aprovação do cadastro.
              </p>
            </div>

            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1">Nome Completo *</label>
              <input
                type="text"
                required
                value={nome}
                onChange={(e) => setNome(e.target.value)}
                placeholder="Ex. José Ribamar da Silva"
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm bg-white text-neutral-800 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                id="reg-nome"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1">CPF (apenas números para login) *</label>
              <input
                type="text"
                required
                value={cpf}
                onChange={handleCpfChange}
                placeholder="000.000.000-00"
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm bg-white text-neutral-800 focus:outline-none focus:ring-1 focus:ring-emerald-500 font-mono"
                id="reg-cpf"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1">E-mail (Opcional - para notificações)</label>
              <input
                type="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                placeholder="jose.silva@email.com"
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm bg-white text-neutral-800 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                id="reg-email"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-neutral-700 mb-1">Senha de Acesso *</label>
              <input
                type="password"
                required
                value={senha}
                onChange={(e) => setSenha(e.target.value)}
                placeholder="Defina uma senha"
                className="w-full px-3 py-2 border border-neutral-300 rounded-lg text-sm bg-white text-neutral-800 focus:outline-none focus:ring-1 focus:ring-emerald-500 font-mono"
                id="reg-senha"
              />
            </div>

            {/* Foto Biometria Panel */}
            <div className="p-4 bg-neutral-50 rounded-xl border border-neutral-200 space-y-3">
              <div className="flex justify-between items-center">
                <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wide">
                  Foto para Biometria Facial *
                </label>
                {fotoBiometria ? (
                  <span className="text-[9px] bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded font-black font-mono border border-emerald-150">
                    FOTO SALVA ✓
                  </span>
                ) : (
                  <span className="text-[9px] bg-rose-100 text-rose-800 px-2 py-0.5 rounded font-black font-mono border border-rose-150">
                    PENDENTE 📸
                  </span>
                )}
              </div>

              {fotoBiometria ? (
                <div className="flex items-center gap-4 bg-white p-3 rounded-lg border border-neutral-150 shadow-xs">
                  <div className="relative w-16 h-16 rounded-full border-2 border-emerald-500 bg-neutral-100 overflow-hidden shrink-0 flex items-center justify-center">
                    <img 
                      src={fotoBiometria} 
                      alt="Biometria captured" 
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-neutral-800">Foto registrada!</p>
                    <p className="text-[10px] text-neutral-500 mt-0.5">Sua imagem está salva para aprovação do RH.</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setIsCapturing(true)}
                    className="px-3 py-1.5 border border-neutral-300 hover:bg-neutral-50 text-neutral-700 text-xs font-bold rounded-lg transition active:scale-95 flex items-center gap-1 shrink-0 cursor-pointer mini-touch-target"
                  >
                    🔄 Mudar
                  </button>
                </div>
              ) : (
                <div className="text-center bg-white p-4 rounded-lg border-2 border-dashed border-neutral-200">
                  <p className="text-[11px] text-neutral-500 mb-3 leading-normal">
                    A biometria facial é um requisito de segurança obrigatório para que você possa retirar suas marmitas de forma autônoma.
                  </p>
                  <button
                    type="button"
                    onClick={() => setIsCapturing(true)}
                    className="px-4 py-2.5 bg-neutral-900 hover:bg-neutral-800 text-white text-xs font-extrabold rounded-lg transition active:scale-95 inline-flex items-center justify-center gap-2 shadow-sm cursor-pointer touch-target w-full"
                  >
                    <Camera className="h-4 w-4 text-emerald-400" /> Abrir Câmera e Tirar Foto 🤳
                  </button>
                </div>
              )}
            </div>

            <div className="pt-4 flex justify-end gap-3 border-t border-neutral-100">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 border border-neutral-300 text-neutral-700 rounded-lg text-sm hover:bg-neutral-50 transition-colors"
                id="cancel-reg-btn"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold shadow transition-colors flex items-center gap-1.5"
                id="submit-reg-btn"
              >
                <UserPlus className="h-4 w-4" /> Solicitar Cadastro
              </button>
            </div>
          </form>
        )}
      </div>

      {/* Camera Capture Modal Overlay */}
      {isCapturing && (
        <CameraCapture
          onCapture={(base64) => {
            setFotoBiometria(base64);
            setIsCapturing(false);
          }}
          onCancel={() => setIsCapturing(false)}
          title="Cadastro de Selfie - SGR"
          subTitle="Posicione seu celular ou tablet em frente à sua face para registrar de forma legível"
        />
      )}
    </div>
  );
}
