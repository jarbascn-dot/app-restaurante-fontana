...
// CORREÇÃO CRÍTICA: Não restaurar token marcado como inválido.
// Se precisaAtivarNotificacao=true, o token em usuarios.fcmToken está expirado.
// Restaurá-lo para fcmTokens criaria um loop infinito: cron tenta → falha →
// deleta fcmTokens → próximo cron restaura mesmo token expirado → falha novamente.
// O sistema aguarda o app renovar o token via autoRecoverFCMToken.
if (userData?.precisaAtivarNotificacao === true) {
console.warn(`[FCM] Token de ${userId} marcado como inválido (precisaAtivarNotificacao=true). Aguardando renovação pelo app. Ignorando fallback.`);
return;
}
...
if (userInvalido?.id) {
// CORREÇÃO CRÍTICA: Limpar también o fcmToken em usuarios para que o fallback
// não restaure o token expirado na próxima execução do cron.
// Ao zerar fcmToken + marcar precisaAtivarNotificacao=true, o fallback ignora
// esse usuário e aguarda o app gerar um token novo via autoRecoverFCMToken.
