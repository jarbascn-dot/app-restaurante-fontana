...
// Read firebase config from JSON file (avoids import attribute issues across Node versions)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
                  ...
                  // FIX: Use doc.id as the userId key (document ID = userId), not data.userId field
                  const fcmSnapshot = await db.collection('fcmTokens').get();
                      const tokenMap: Record<string, string> = {};
                      fcmSnapshot.forEach((tokenDoc) => {
                                                  const data = tokenDoc.data();
                                                  // Document ID is the userId; also support legacy data.userId field
                                                              const uid = tokenDoc.id || data.userId;
                                                  if (data.token && uid) {
                                                                                      tokenMap[uid] = data.token;
                                                  }
                      });
